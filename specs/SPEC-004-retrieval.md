...
15. **The full-text branch is deleted** *(2026-08-05, owner decision, after SPEC-007 KD-12's confirmatory comparison)*.
...
16. **PROPOSED, NOT APPLIED — expose cosine similarity as `score`** *(2026-08-05, raised in the deletion's review)*.
...
- **Cross-spec note (binding on SPEC-007) — a retrieval-only evaluation set, separate from the 50-question golden set** *(added 2026-07-26, second review)*. Key decision 12 cannot be settled with the artifacts currently planned, and the fix is more questions of a *cheaper kind*, not a bigger corpus (SPEC-003 KD-13).
  ...
  - ~~**The existing 26-question smoke set is the seed of the retrieval set**, not a third artifact to maintain.~~ **SUPERSEDED 2026-08-04 by SPEC-007 KD-12 amendment 5 (owner decision).** The smoke set is **excluded** from the confirmatory retrieval set and kept as a **regression** set. The reason is the property this note could not have known at the time: all 26 were authored *with the expected section in hand*, which pilot-1 identified as what makes them lexical bullseyes and what saturated `recall@8` at 1.000. At the approved N = 150 they would be 17 % of the instrument, so the cost of "not a third artifact to maintain" is a known bias in a sixth of the cases — and the maintenance it saves is four hours of authoring, once.
