...
src/rag_qa/generation/
    ...
    pricing.py        # per-identity token pricing, point-in-time cost
...
- **Service + `query_log` tests** (AC-8, AC-9, AC-10) run against the dockerized Postgres. Migration 0004 follows SPEC-002's scratch-database pattern, seeding a pre-migration `query_log` row to prove the new not-null columns are added without data loss.
