...
**Review amendment (2026-07-26, approving round):** the server-side taxonomy is not defined here. It is derived from `rag_qa/api/conditions.py`, the shared registry introduced by SPEC-006 Key decision 16's reconciliation (commit `e81d569`) — see Key decision 7. The review that approved this spec required it: *"that taxonomy and the client-facing states should not be two independent lists."*
...
```
src/rag_qa/api/
    middleware.py         # + the completion record, in RequestContextMiddleware (KD-7)
